 ...
 * @author Diogo
 */
public class UnidadeConstrutor implements IUnidade, IAtacavel {
    ...
    public UnidadeConstrutor(int x, int y, Cidade cidade) {
        this.posX = x;
        this.posY = y;
        this.forcaCombate = 10;
        this.vida = 1;
   ...
   /**
    * Constrói uma estrada em um terreno especificado.
    *
    * Este método verifica se o terreno é válido e se já possui uma estrada 
    * construída. Se o terreno for válido e não tiver uma estrada, o símbolo 
    * da estrada é adicionado ao terreno e uma mensagem de confirmação é exibida.
    *
    * @param mapa o mapa onde a estrada será construída (não utilizado diretamente neste método)
    * @param terreno o terreno onde a estrada será construída
    */
   public void construirEstrada(IMapa mapa, ITerreno terreno) {
       if (terreno == null) {
           GestorMensagens.mostrarMensagem("Terreno invalido!");
           return;
       }
       if (terreno.procuraOverlay(ITerreno.SIMBOLO_ESTRADA)) {
           GestorMensagens.mostrarMensagem("Terreno ja tem estrada construida!");
           return;
       }
       // Adicionar sobreposição da estrada
       terreno.setOverlaySimbolo(ITerreno.SIMBOLO_ESTRADA);
       GestorMensagens.mostrarMensagem("Estrada construida em (" + terreno.getX() 
               + ", " + terreno.getY() + ").");
   }
